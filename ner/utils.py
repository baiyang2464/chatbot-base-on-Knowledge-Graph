...
class  DATAPROCESS:
    def __init__(self,train_data_path,train_label_path,test_data_path,test_label_path,word_embedings_path,vocb_path,seperate_rate=0.1,batch_size=100,
				 ...
				 'B-che':7,'I-che':8,'E-che':9,
				 'B-dru':10,'I-dru':11,'E-dru':12,
				 'S-dis':13,'S-sym':14,'S-che':15,'S-dru':16}):
    ...
    def handleInputData(self,text):
        ...
        data_line = list(text.strip())
        sumCount = len(data_line)//self.sentence_length
        if sumCount * self.sentence_length < len(data_line): sumCount+=1
        for count in range(sumCount-1):
        ...
        for idx in range(len(input_data_raw)):
			efficient_sequence_length.append(min(self.sentence_length,len(input_data_raw[idx])))
            #填充
            data = self.pad_sequence(input_data_raw[idx], self.sentence_length, 0)
            output_x.append(data)
        return data_line,output_x,efficient_sequence_length
...
id2state={0:'O',
		  ...
		  7:'B-che',8:'I-che',9:'E-che',
		  10:'B-dru',11:'I-dru',12:'E-dru',
		  13:'S-dis',14:'S-sym',15:'S-che',16:'S-dru'}

def count_entity(labels,lens):
#输入是一个句子的标签
    start = -1#一个实体的起始位置
    size =0  #实体的长度
    rst = set()
    for i in range(lens):
        _state = id2state[labels[i]]
        ...
        if _state[0]=='E' or _state[0]=='S':
            ...
            start=-1
            size=0
        if start>=0 and _state[0]=='B' or _state[0]=='O':
            rst.add((labels[start],start,size))
            start=-1
            size=0
    if start>=0:
...
# predict_labels:[batch_size,sequence_length],real_labels:[batch_size,sequence_length]
    ...
    for sentence_index in range(sentence_nums):
        try:
            predict_set=count_entity(predict_labels[sentence_index],efficient_length[sentence_index])
            real_set=count_entity(real_labels[sentence_index],efficient_length[sentence_index])
